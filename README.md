...
* 반응형 웹 사이트
1. 동영상 삽입 : 메인 페이지에 동영상을 삽입하여 사용자에게 동적인 경험 제공.
2. AI 협업 기반 동적 애니메이션 : 이미지 변형 (타일, 순차적) 애니메이션 구현
3. 스크롤 애니메이션 적용 : AOS 플러그인을 사용하여 스크롤에 반응하는 애니메이션 효과 적용.
4. 다양한 스와이퍼 활용 : 다양한 섹션에서 Swiper를 활용하여 콘텐츠 슬라이드 및 캐러셀 기능 구현.
5. 갤러리 레이아웃 구성 : 갤러리 페이지에 Masonry(메이슨리) 라이브러리를 사용하여
   유기적인 그리드 (벽돌 쌓기) 레이아웃 구성.
6. 유튜브 영상 팝업 기능 : Magnific Popup 플러그인을 활용하여 반응형 팝업(iframe)으로 구현.
