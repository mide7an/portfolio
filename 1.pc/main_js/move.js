...
$(document).ready(function () {
  ...
  $('.gallery .link1 span').delay(1500).animate({ top: 410, opacity: 1 }, 'slow');
  ...
  function moveg() {
    ...
    $('.gallery li span').css('top', 440).css('opacity', 0);
    $('.gallery .link' + cnt).find('span').delay(1500).animate({ top: 410, opacity: 1 }, 'slow');
  ...
  $('.mbutton').click(function (event) {  //각각의 버튼 클릭시
    var $target = $(this); //클릭한 버튼 $target == $(this)  var $target=$(event.target);  자바스크립트 변수는 $ 가능
    ...
    // 	 cnt=5; 
    // } 
    //  해당버튼 클릭시 인덱스번호+1 을 cnt로 변환
    var ind = $(this).index('.mbutton');
    cnt = ind + 1;
    ...
    if (onoff == false) { //중지상태냐??
      onoff = true; //동작~~
      $('.ps').html('<span class="hidden">stop</span><i class="fa-solid fa-stop"></i></i>');
    }
  });

  //stop/play 버튼 클릭시 타이머 동작/중지
  $('.ps').click(function () {
    if (onoff == true) { // 타이머가 동작 중이냐??  가정법
      clearInterval(timeonoff);   //살인마 고용 stop버튼 클릭시
      $(this).html('<span class="hidden">play</span><i class="fa-solid fa-play"></i></i>'); //js파일에서는 경로의 기준이 html파일이 기준!!
      onoff = false;
    } else {  // false 타이머가 중지 상태냐??
      timeonoff = setInterval(moveg, 4000); //play버튼 클릭시  부활
      $(this).html('<span class="hidden">stop</span><i class="fa-solid fa-stop"></i></i>');
      onoff = true;
    }
  });
  ...
  $('.visual .btn').click(function () {
    ...
    if (onoff == false) {  /* 타이머중지상태면 ... */
      onoff = true; /* 동작상태로 만들어준다 */
      $('.ps').html('<span class="hidden">stop</span><i class="fa-solid fa-stop"></i></i>');
    }
  });
});
